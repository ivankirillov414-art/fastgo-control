...
export function messageFor(error){const raw=String(error?.message||error);const known={daily_expired:'Дедлайн прошёл. Этот дейлик уже нельзя отправить.',daily_not_active:'Задание уже завершено или находится на проверке.',quest_locked:'Сначала заверши предыдущий узел этой ветки.',file_too_large:'Файл больше 20 МБ.',invalid_device:'Личный ключ не распознан. Открой настройки.',verification_required:'Нужно отправить доказательство выполнения.',rate_limited:'Слишком много отправок. Попробуй чуть позже.'};return known[raw]||(/Failed to fetch|NetworkError|fetch failed|abort|timeout/i.test(raw)?'Нет связи с сервером. Проверь интернет и повтори.':raw);}
export async function request(base,path,{device,fetcher=fetch,timeout=20000,...options}={}){const ctrl=new AbortController();const timer=setTimeout(()=>ctrl.abort(),timeout);try{const r=await fetcher(base+path,{...options,signal:ctrl.signal,cache:'no-store',headers:{'x-device-id':device,...options.headers}});let j;try{j=await r.json();}catch{throw Error('Сервер вернул непонятный ответ. Повтори позже.');}if(!r.ok)throw Error(j.error||`Ошибка сервера (${r.status}).`);return j;}finally{clearTimeout(timer);}}
