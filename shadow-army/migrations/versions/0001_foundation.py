...
agent_rank = postgresql.ENUM("E", "D", "C", "B", "A", "S", name="agentrank", create_type=False)
project_status = postgresql.ENUM("IDEA", "ACTIVE", "BLOCKED", "COMPLETED", "CANCELLED", name="projectstatus", create_type=False)
memory_type = postgresql.ENUM("FACT", "HYPOTHESIS", "DECISION", "RESULT", "LESSON", "RESOURCE", name="memorytype", create_type=False)
approval_status = postgresql.ENUM("WAITING", "APPROVED", "REJECTED", "CONSUMED", name="approvalstatus", create_type=False)


def upgrade() -> None:
    bind = op.get_bind()
    for enum_type in (agent_rank, project_status, memory_type, approval_status):
    ...
    op.create_index("ix_relations_from_id", "relations", ["from_id"]); op.create_index("ix_relations_relation", "relations", ["relation"]); op.create_index("ix_relations_to_id", "relations", ["to_id"])
    ...
    op.create_index("ix_events_created_at", "events", ["created_at"]); op.create_index("ix_events_actor", "events", ["actor"]); op.create_index("ix_events_action", "events", ["action"])
...
def downgrade() -> None:
    op.drop_table("approvals"); op.drop_table("events"); op.drop_table("relations"); op.drop_table("memories"); op.drop_table("projects"); op.drop_table("agents")
    bind = op.get_bind()
    for enum_type in (approval_status, memory_type, project_status, agent_rank): enum_type.drop(bind, checkfirst=True)
