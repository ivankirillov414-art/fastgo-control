...
test('busy lock reports retryable 503 without entering the critical section',()=>{const h=harness();h.lock();assert.throws(()=>h.run('sale',h.sale()),e=>e.status===503&&/База занята/.test(e.message));h.unlock();assert.equal(h.rows('Продажи').length,0);});
...
test('HTTP result loss remains retryable and preserves the committed sale receipt',()=>{const h=harness(),p=h.sale();h.props.set('FASTGO_API_SECRET','test-secret');h.context.ContentService={MimeType:{JSON:'json'},createTextOutput:body=>({setMimeType:()=>JSON.parse(body)})};const event={postData:{contents:JSON.stringify({secret:'test-secret',action:'sale',params:p,actor:owner})}};h.setFault('after');const failed=h.context.doPost(event);assert.equal(failed.status,503);assert.match(failed.error,/эту же операцию/);const retry=h.context.doPost(event);assert.ok(retry.data.id);assert.equal(h.fetches(),1);assert.equal(h.rows('Продажи').length,1);assert.equal(h.rows('Товары')[0]['Остаток, шт.'],0);});
test('each Google response has a fresh delivery id without changing business data',()=>{const h=harness();h.context.ContentService={MimeType:{JSON:'json'},createTextOutput:body=>({setMimeType:()=>JSON.parse(body)})};const first=h.context.json_({data:{value:1}}),second=h.context.json_({data:{value:1}});assert.deepEqual(first.data,second.data);assert.notEqual(first.response_id,second.response_id);});
