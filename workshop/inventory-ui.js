...
const BARCODE_URL='https://cdn.jsdelivr.net/npm/jsbarcode@3.12.3/dist/JsBarcode.all.min.js';
let me=null;
...
async function scanCode(title='Сканировать штрих-код'){
  return new Promise(async resolve=>{
    ...
    try{
      ...
      if(!done)$('scan-status').textContent='Камера активна';
    }catch(e){
...
async function newPartForm(prefill=''){
  const c=await getCatalog();const categories=[...new Set((c.categories||[]).map(x=>x.name).filter(Boolean))].sort();
  const options=categories.map(x=>`<option value="${esc(x)}"></option>`).join('');
  const d=showDialog('Новая товарная позиция',`<form id="new-part-form" class="inv-form"><label>Категория<input name="category" list="part-categories" required placeholder="Подшипники"><datalist id="part-categories">${options}</datalist></label><label>Наименование<input name="name" required placeholder="Подшипник рулевой 2008 2RS"></label><label>Модель / размер<input name="model" placeholder="2008 2RS"></label><label>Артикул / SKU<input name="sku" value="${esc(prefill&&!String(prefill).startsWith('FGP-')?prefill:'')}"></label><div class="inv-grid"><label>Закупка, ₽<input name="unit_cost" type="number" min="0" step="0.01" value="0"></label><label>Продажа, ₽<input name="retail_price" type="number" min="0" step="0.01" value="0" required></label><label>Количество<input name="initial_quantity" type="number" min="0" step="1" value="0"></label><label>Ед.<input name="unit" value="шт"></label></div><label>Основное фото<input name="photo" type="file" accept="image/jpeg,image/png,image/webp" capture="environment"></label><label class="inv-check"><input name="category_primary" type="checkbox"> Сделать это фото основным и для категории</label><p class="muted">Штрих-код FastGo будет присвоен автоматически после сохранения.</p></form>`,`<button class="btn ghost" data-inv-close>Отмена</button><button class="btn" id="new-part-save">Создать и принять</button>`);
  $('new-part-save').onclick=async()=>{
    ...
    try{
      ...
      const part=await api('part_save',{name:f.get('name'),category:f.get('category'),model:f.get('model'),sku:f.get('sku'),unit_cost:Number(f.get('unit_cost')||0),retail_price:Number(f.get('retail_price')||0),unit:f.get('unit')||'шт'});
      if(initial>0)await api('stock',{part_id:part.id,movement_type:'receipt',quantity:initial,note:'Первичная ручная приёмка',request_id:crypto.randomUUID()});
...
function renderCart(){
  ...
  const btn=$('sale-complete');if(btn)btn.disabled=!rows.length;
}
...
async function openSales(){
  if(!me)me=await api('me');if(!['owner','admin','receiver','manager'].includes(me.role))return notice('У вашей роли нет доступа к продажам','bad');
  const c=await getCatalog(true);const parts=(c.parts||[]).filter(x=>x.active!==false);
  ...
  $('sale-scan').onclick=async()=>{const code=await scanCode('Сканирование продажи');if(!code)return;try{addToCart(await partByCode(code));}catch(e){notice(e.message,'bad');}finally{if(!ownDialog().open)openSales();}};
  renderCart();
  $('sale-complete').onclick=async()=>{
    if(!cart.size)return;const btn=$('sale-complete');btn.disabled=true;
    try{const result=await api('sale',{request_id:crypto.randomUUID(),payment_method:$('sale-payment').value,note:$('sale-note').value,items:[...cart.values()].map(x=>({part_id:x.part.id,quantity:x.qty}))});cart.clear();catalogCache=null;closeDialog();notice(`Продажа №${result.sale_number} · ${money(result.total)}`);}catch(e){notice(e.message,'bad');btn.disabled=false;}
  };
}
  ...
  try{
    ...
    if(location.hash.startsWith('#stock')){
      ...
      if(table&&!table.dataset.barcodeEnhanced){
        ...
        table.querySelectorAll('tbody tr').forEach(tr=>{const name=tr.querySelector('td b')?.textContent||'';const p=map.get(name);if(!p)return;const small=tr.querySelector('td small');if(small)small.innerHTML=`${esc(p.sku||'Без артикула')} · <b>${esc(p.barcode)}</b>`;tr.addEventListener('dblclick',()=>printPartLabel(p).catch(e=>notice(e.message,'bad')));});
      }
    }
...
window.addEventListener('workshop-auth-required',()=>{me=null;catalogCache=null;cart.clear();});
