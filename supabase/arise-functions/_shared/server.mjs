...
const H={'Access-Control-Allow-Origin':'*','Access-Control-Allow-Headers':'content-type,x-device-id','Access-Control-Allow-Methods':'GET,POST,OPTIONS','Content-Type':'application/json','Cache-Control':'no-store'};
...
function database(env,fetcher){const origin=env.get('SUPABASE_URL'),key=env.get('SUPABASE_SERVICE_ROLE_KEY');if(!origin||!key)throw Error('server_not_configured');const headers={apikey:key,Authorization:`Bearer ${key}`};return {
 async rest(table,query={},method='GET',body,prefer='return=representation'){const url=new URL(origin+'/rest/v1/'+table);for(const [k,v] of Object.entries(query))url.searchParams.set(k,String(v));const response=await fetcher(url,{method,headers:{...headers,'Content-Type':'application/json',Prefer:prefer},body:body===undefined?undefined:JSON.stringify(body),signal:AbortSignal.timeout(15000)});if(!response.ok)throw Error('database_error');if(response.status===204)return null;return response.json();},
...
export function createHandler(action,{env=globalThis.Deno?.env,fetcher=fetch,now=()=>new Date()}={}){return async req=>{
 ...
 try{
  ...
  const device=(req.headers.get('x-device-id')||'').trim();if(!devicePattern.test(device))return json({error:'invalid_device'},401);
  ...
  if(action==='rpg-progress'){if(req.method!=='GET')return json({error:'method_not_allowed'},405);const p=await sync(),done=p.completed_quests||[];return json({xp:p.xp||0,completed_quests:done,daily_xp:p.daily_xp||0,next_quest:QUESTS.find(q=>q.track==='fusion'&&!done.includes(q.id))?.id??null,updated_at:p.updated_at});}
 ...
 }catch(error){const message=String(error?.message||error),allowed=['file_required','file_too_large','unsupported_file','invalid_quest','daily_expired','daily_not_active','server_not_configured','database_error','upload_failed'];return json({error:allowed.includes(message)?message:'request_failed'},message==='file_too_large'?413:message==='database_error'||message==='server_not_configured'?503:400);}
};}
